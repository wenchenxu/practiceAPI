    ...
    <tbody class="divide-y divide-slate-200 bg-white">
        @forelse ($vehicles as $vehicle)
            <tr class="hover:bg-slate-50 transition-colors duration-150 ease-in-out">
                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-center font-medium sm:pl-6">
                    <button type="button" class="text-sky-600 hover:text-sky-800 open-edit-modal-button"
                            data-vehicle='{!! json_encode($vehicle->toArray()) !!}' {{-- Pass all vehicle data --}}
                            data-update-url="{{ route('vehicles.update', $vehicle->id) }}"
                            title="Edit Vehicle">
